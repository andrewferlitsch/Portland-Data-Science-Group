technical.controller( 'agileCtrl', function( $scope, $http ) {
	$scope.subject 	= "Agile";
...
.directive( "questionsAgile", function() {
	return {
		...
		template: "<a name='agile'/>" +
				  ...
				  "<ul>" +
				  "	<li ng-repeat='question in random'><span class='w3-tooltip rank' name='agile' id='{{question.id}}'>Rank {{question.rank}} <span class='w3-text w3-tag w3-khaki w3-round w3-small' ng-click='rank(question.id);'>Click to Suggest Another Ranking</span></span> {{question.question}}<br/><br/>" +
